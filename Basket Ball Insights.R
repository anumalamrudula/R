...
{
  Data <- data[rows,,drop=F]    # Turns to a vector
...
# Salary
myplot(Salary)
myplot(Salary/Games)        # Because of injuries
myplot(Salary/FieldGoals)   # Same
...
#If we recall, Dwight Howard's Field Goal Accuracy was exceptional:
myplot(FieldGoals/FieldGoalAttempts)
...
#1. We can see how players' preference for 2 or 3 point shots changes throughout their career.
...
#2. There is one exception. We can see that one player has not changed his style at all - almost always scoring only 2-pointers - It's Dwight Howard! 
#   Now that explains a lot. The reason that Dwight Howard's Field Goal accuracy is so good is because he almost always scores 2-pointers only.
#   That means he can be close to the basket or even in contact with it.
#   Free throws, on the other hand require the player to stand 15ft (4.57m) away from the hoop. That's probably why Dwight Howard's Free Throw Accuracy is poor.
